...
[Kubernetes backend] Both service_account and load_config_file were set to true.
load_config_file takes precedence so service_account settings could be overwritten.
`
)
...
func getInitialConfig(data *schema.ResourceData) (*restclient.Config, error) {
	...
	sa := data.Get("service_account").(bool)
